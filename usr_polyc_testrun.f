	  ...
	  PROGRAM MAIN
	  IMPLICIT NONE
	  INTEGER i,HCP_slipSym,BCC_slipSym,cols 
	  INTEGER NODESTOT,MXNONP,NELEMSTOT,NUMDEPTOT 
	  parameter(HCP_slipSym=30,BCC_slipSym=12,NODESTOT=256,NUMDEPTOT=130,MXNONP=4,NELEMSTOT=736)
	  DOUBLE PRECISION m_alpha_hcp(HCP_slipSym,3),n_alpha_hcp(HCP_slipSym,3)
	  DOUBLE PRECISION m_alpha_bcc(BCC_slipSym,3),n_alpha_bcc(BCC_slipSym,3)
	  DOUBLE PRECISION Dmat(6,6),s_alpha_t_hcp(HCP_slipSym),s_alpha_t_bcc(BCC_slipSym)
	  DOUBLE PRECISION F_tau(3,3),FP_t(3,3),FE_t(3,3)	  
	  DOUBLE PRECISION FP_tau(3,3),FE_tau(3,3),T_tau(3,3)	
	  DOUBLE PRECISION ODF(NODESTOT-NUMDEPTOT),RZTX(NODESTOT*3)
	  INTEGER NODTX(1:MXNONP,1:NELEMSTOT), NODMAP(1:2,1:NUMDEPTOT)
	  
	  call Read_Inputs(HCP_slipSym,BCC_slipSym,NODESTOT,MXNONP,NELEMSTOT,NUMDEPTOT,& !!inputs
     &	m_alpha_hcp,n_alpha_hcp,m_alpha_bcc,n_alpha_bcc,Dmat,s_alpha_t_hcp,s_alpha_t_bcc,F_tau,FP_t,FE_t,& !!outputs
	 &	ODF,RZTX,NODTX,NODMAP) !!outputs
...
! ██║  ██║███████╗██║  ██║██████╔╝    ██║██║ ╚████║██║     ╚██████╔╝   ██║   ███████║
! ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═════╝     ╚═╝╚═╝  ╚═══╝╚═╝      ╚═════╝    ╚═╝   ╚══════╝                                                                               
	  SUBROUTINE Read_Inputs(HCP_slipSym,BCC_slipSym,NODESTOT,MXNONP,NELEMSTOT,NUMDEPTOT,& !!inputs
     &	m_alpha_hcp,n_alpha_hcp,m_alpha_bcc,n_alpha_bcc,Dmat,s_alpha_t_hcp,s_alpha_t_bcc,F_tau,FP_t,FE_t,& !!outputs
	 &	ODF,RZTX,NODTX,NODMAP) !!outputs
	  IMPLICIT NONE
	  INTEGER i,HCP_slipSym,BCC_slipSym,cols,nrow
	  INTEGER NODESTOT,MXNONP,NELEMSTOT,NUMDEPTOT
	  DOUBLE PRECISION m_alpha_hcp(HCP_slipSym,3),n_alpha_hcp(HCP_slipSym,3)
	  DOUBLE PRECISION m_alpha_bcc(BCC_slipSym,3),n_alpha_bcc(BCC_slipSym,3)
	  DOUBLE PRECISION Dmat(6,6),s_alpha_t_hcp(HCP_slipSym),s_alpha_t_bcc(BCC_slipSym)
	  ...
	  character(len=100) :: filename,header
		...
		cols=3
		nrow=HCP_slipSym
		filename='input_files/HCP_Inputs.DAT' 
		write(*,*) filename
		open  ( unit = 1, file = filename, status='old',form='formatted')
			...
			do i=1,nrow
				read (1,*) m_alpha_hcp(i,1:cols)
				write(*,*) m_alpha_hcp(i,1:cols)
			enddo
			
			read (1,*) header
			write(*,*) 'Reading:',header
			do i=1,nrow
				read(1,*) n_alpha_hcp(i,1:cols)
				write(*,*) n_alpha_hcp(i,1:cols)
			enddo

			read (1,*) header
			write(*,*) 'Reading:',header
			read(1,*) s_alpha_t_hcp(1:HCP_slipSym)
			write(*,*) s_alpha_t_hcp(1:HCP_slipSym)		
		
		close(1)

		nrow=BCC_slipSym
		filename='input_files/BCC_Inputs.DAT' 
		write(*,*) filename
		open  ( unit = 1, file = filename, status='old',form='formatted')
			...
			write(*,*) 'Reading:',header
			do i=1,nrow
				read (1,*) m_alpha_bcc(i,1:cols)
				write(*,*) m_alpha_bcc(i,1:cols)
			enddo
			
			read (1,*) header
			write(*,*) 'Reading:',header
			do i=1,nrow
				read(1,*) n_alpha_bcc(i,1:cols)
				write(*,*) n_alpha_bcc(i,1:cols)
			enddo

			read (1,*) header
			write(*,*) 'Reading:',header
			read(1,*) s_alpha_t_bcc(1:BCC_slipSym)
			write(*,*) s_alpha_t_bcc(1:BCC_slipSym)		
		
		close(1)

		filename='input_files/Common_Inputs.DAT' 
		write(*,*) filename
		open  ( unit = 1, file = filename, status='old',form='formatted')
			...
			cols=3
			do i=1,nrow
				read(1,*) F_tau(i,1:cols)
				write(*,*) F_tau(i,1:cols)
			enddo		

			read (1,*) header
			write(*,*) 'Reading:',header
			do i=1,nrow
				read(1,*) FP_t(i,1:cols)
				write(*,*) FP_t(i,1:cols)
			enddo	
			...
			write(*,*) 'Reading:',header
			do i=1,nrow
				read(1,*) FE_t(i,1:cols)
				write(*,*) FE_t(i,1:cols)
			enddo	
		
		close(1)			

		filename='input_files/ODF.DAT' 
		write(*,*) filename
		open  ( unit = 1, file = filename, status='old',form='formatted')		
		read(1,*) ODF(1:SUM(NUMIND(1:2)))
		write(*,*) ODF(1:SUM(NUMIND(1:2)))
		close(1)

		filename='input_files/RZTX.DAT' 
		write(*,*) filename
		open  ( unit = 1, file = filename, status='old',form='formatted')		
		read(1,*) RZTX(1:NODESTOT*3)
		write(*,*) RZTX(1:NODESTOT*3)
		close(1)

		
		filename='input_files/NODTX.DAT' 
		write(*,*) filename
		open  ( unit = 1, file = filename, status='old',form='formatted')		
		read(1,*) NODTX(1:MXNONP,1:NELEMSTOT)
		write(*,*) NODTX(1:MXNONP,1:NELEMSTOT)
		close(1)
		
		filename='input_files/NODMAP.DAT' 
		write(*,*) filename
		open  ( unit = 1, file = filename, status='old',form='formatted')		
		read(1,*) NODMAP(1:2,1:NUMDEPTOT)
		write(*,*) NODMAP(1:2,1:NUMDEPTOT)
		close(1)		
